...
async function procesarLogin(nombreUsuario, contrasena, rol) {
    ...
    try {
        ...
        estadoApp.asignaciones = datos.usuario.asignaciones || []; 
...
function agregarFilaAsignacion(gradoVal = '4', seccionVal = '') {
...
// Verificar si ya vio las instrucciones
function iniciarActividad(tipo) {
    const usuario = estadoApp.nombreUsuario || 'invitado';
    const vioInstrucciones = localStorage.getItem(`instrucciones_vistas_${usuario}_${tipo}`);
    ...
    ejecutarInicioActividad(tipo);
}

function actualizarVideoInstrucciones(tipo) {
    const video = document.getElementById('video-instrucciones');
    if (video) {
        const nombreVideo = tipo.toLowerCase();
        video.src = `assets/videos/${nombreVideo}.mp4`;
        video.load();
    }
}

function mostrarInstrucciones(tipo) {
    estadoApp.actividadPendiente = tipo;
    const modal = document.getElementById('modal-instrucciones');
    const titulo = document.getElementById('instrucciones-titulo');
    
    if (titulo) titulo.textContent = `Instrucciones: ${tipo.charAt(0).toUpperCase() + tipo.slice(1)}`;
    
    actualizarVideoInstrucciones(tipo);
    
    const btn = document.getElementById('btn-comenzar-juego');
    if (btn) {
        btn.onclick = () => cerrarInstrucciones(true);
        btn.innerHTML = '<img src="assets/svg/check.svg"><p>¡Entendido, a jugar!</p>';
    }
...
function cerrarInstrucciones(guardar = false) {
    const modal = document.getElementById('modal-instrucciones');
    ...
    if (guardar && estadoApp.actividadPendiente) {
        const usuario = estadoApp.nombreUsuario || 'invitado';
        localStorage.setItem(`instrucciones_vistas_${usuario}_${estadoApp.actividadPendiente}`, 'true');
        ejecutarInicioActividad(estadoApp.actividadPendiente);
        estadoApp.actividadPendiente = null;
    }
}

function mostrarInstruccionesManual() {
    estadoApp.actividadPendiente = null;
    const tipo = estadoApp.actividadActual;
    const modal = document.getElementById('modal-instrucciones');
    const titulo = document.getElementById('instrucciones-titulo');
    ...
    // Cambiar texto del botón para que no diga "A jugar" si ya está jugando
    const btn = document.getElementById('btn-comenzar-juego');
    const textoOriginal = btn.innerHTML;
    
    btn.innerHTML = '<img src="assets/svg/check.svg"><p>Continuar</p>';
    btn.onclick = () => {
        cerrarInstrucciones(false);
        setTimeout(() => {
...
function abrirTutorialDesdeMenu(event, tipo) {
    if (event) {
    ...
    btn.innerHTML = '<img src="assets/svg/check.svg"><p>Cerrar</p>';
    btn.onclick = () => {
        cerrarInstrucciones(false);
        setTimeout(() => {
             btn.innerHTML = textoOriginal;
...
function ejecutarInicioActividad(tipo) {
...
function generarEjercicioAdaptativo(tipo, nivel) {
    ...
    const rangos = {
        ...
        multiplicacion: [
            { minimo: 2, maximo: 3 }, { minimo: 2, maximo: 4 }, { minimo: 2, maximo: 5 },
            { minimo: 2, maximo: 6 }, { minimo: 2, maximo: 7 }, { minimo: 2, maximo: 8 },
            { minimo: 2, maximo: 9 }, { minimo: 2, maximo: 10 }, { minimo: 3, maximo: 10 },
        ],
    };
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const btnCerrarInstr = document.getElementById('btn-cerrar-instrucciones');
    if (btnCerrarInstr) btnCerrarInstr.addEventListener('click', () => cerrarInstrucciones(false));

    const btnComenzarJuego = document.getElementById('btn-comenzar-juego');
    if (btnComenzarJuego) btnComenzarJuego.addEventListener('click', () => cerrarInstrucciones(true));
